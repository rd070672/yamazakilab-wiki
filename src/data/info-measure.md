...
計測インフォマティクスの標準的なフローは、概ね次のように整理できる。
...
## 6. 材料・プロセス・物理インフォマティクスとの関係

- 材料インフォマティクス
  - 材料の組成・構造・物性のモデル化を担当。
  - 計測インフォマティクスで抽出された特徴量（スペクトル指標、画像特徴など）を
    「材料の説明変数／応答変数」として利用する。

- プロセスインフォマティクス
  - 熱処理条件・成膜パラメータなど「プロセス条件 ↔ 特性」の関係をモデル化。
  - 計測インフォマティクスは「各条件で取れたデータの質と特徴」を提供。

- 物理インフォマティクス
  - DFT, MD, フェーズフィールドなどの物理モデルと
    データ駆動モデルを橋渡しする役割。
  - 計測インフォマティクスの結果（スペクトル・画像）を、
    物理シミュレーションの出力と比較・同化することで、
    モデルの検証・補正に用いる。
...
- 本ページ「計測インフォマティクス」では、
  - 計測データの取得から前処理・特徴抽出・解析・フィードバックまでの流れを整理し、
  - 材料・プロセス・物理インフォマティクスと役割を分担する形で位置づけた。