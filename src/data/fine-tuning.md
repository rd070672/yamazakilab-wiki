...
- MatGL チュートリアル（M3GNet等の学習・実行の入り口）
  - https://matgl.ai/tutorials.html
- CHGNet 論文（事前学習のデータ源、予測可能量など）
  - https://www.nature.com/articles/s42256-023-00716-3

## 概要

機械学習ポテンシャル（ML potential, MLIP）のファインチューニングとは、既存の学習済みポテンシャル（基盤モデル、ユニバーサルモデル）を出発点にして、研究対象の材料系・相・温度圧力条件・欠陥や表面など、特定用途のデータで追加学習し、精度と安定性を上げる手法である。
...
$$
\mathcal{L} =
w_E \, \mathrm{MSE}(E_{\text{ML}}-E_{\text{ref}})
+
...
## 10. 研究室wikiに残すべきメタデータ（再現性）

- 参照DFT条件（汎関数、U、SOC、ENCUT、k点、収束条件）
- 学習データ内訳（相、欠陥、表面、温度、ひずみ、サンプル数）
- split方針（trajectory単位か、条件単位か）
- 学習設定（loss重み、学習率、凍結層、replay比率、早期終了条件）
- 評価項目と結果（RMSEだけでなく、MD安定性、弾性、フォノン等）