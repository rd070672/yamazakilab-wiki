# 深層学習の基礎と概念の地図
Deep Learning: Foundations and Concepts（Bishop & Bishop）総論（データサイエンス技術文書, 2024）
...
以降では、各章を「ねらい／主要トピック／中心式／データサイエンスとしての意味／次章への接続」の形で詳述する。
...
次章への接続  
確率を基礎に置く理由（損失の意味、生成、潜在変数）を用意する。
...
次章への接続  
標準分布と指数型分布族が、回帰・分類・潜在変数・正則化（事前分布）へ直結する。
...
次章への接続  
単層ネットの回帰・分類は、分布とリンク関数（線形予測子）として理解できるようになる。
...
次章への接続  
分類を確率（ロジスティック、ソフトマックス）として書けるようにする。
...
次章への接続  
深層化により特徴表現を学習し、分類・回帰を同型の枠組みで扱う。
...
次章への接続  
学習は結局、損失の勾配をどう計算し、どう更新するかである。次章で勾配法を扱う。
...
次章への接続  
勾配が計算できる前提があるが、深層モデルでそれを効率良く得る方法が逆伝播である。
...
次章への接続  
学習が進むほど訓練データに合わせすぎる問題が生じる。次章で正則化を扱う。
...
次章への接続  
画像のような格子構造に対して、局所性と重み共有を組み込むCNNへ進む。
...
次章への接続  
次の段階として、より一般の依存構造（系列・グラフ・潜在構造）を確率分布として扱う基礎が必要になる。
...
次章への接続  
系列や集合を扱う代表的アーキテクチャとしてTransformerを扱う。
...
次章への接続  
依存構造をもつ分布の計算では、周辺化や期待値が難しくなるため、サンプリングが重要になる。
...
次章への接続  
連続潜在では微分可能性を活かした学習（再パラメータ化など）が主役になる。
...
次章への接続  
生成モデルの代表系であるGANへ進む。ここでは尤度ではなく対戦的学習が中心となる。
...
次章への接続  
最後に、表現学習の基本枠組みとしてオートエンコーダを整理する。
...
次章への接続  
本書はここで閉じ、確率・最適化・アーキテクチャ・生成が一つの地図に収束した状態を作る。
...
- 第2章と第3章で確率と分布を固める。
- 第4章と第5章で回帰と分類を「分布と損失」に落とし込む。
- 第6章から第9章で、深層化と学習（勾配・逆伝播・正則化）を計算として理解する。
- 第10章、第12章、第13章で、入力構造に適したアーキテクチャの基本を身につける。
- 第11章、第14章から第19章で、分布・推論・生成をまとめて扱い、「分布を学ぶ」視点を完成させる。
