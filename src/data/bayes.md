...
- **PHYSBO（物理・材料向けベイズ最適化ライブラリ）公式マニュアル（日本語）**  
  ...
  - 離散候補（組成リスト、条件グリッド）に強く、材料探索・条件最適化の「まず動かす」に向く。

- **統計数理研究所（ISM）「従属性のあるデータへの不確実性の定量化」**  
  https://www.ism.ac.jp/editsec/toukei/28.html  
  - ベイズ推論・MCMCを含む「不確かさの扱い」の導入として読みやすい。

- **神戸大学 分寺杏介「ベイズ統計 イントロダクション」講義資料（PDF）**  
  https://www2.kobe-u.ac.jp/~bunji/files/lecture/bayes/bayes-01-introduction.pdf  
  - ベイズの定理〜事前/尤度/事後の整理に強い。

- **材料分野のベイズ最適化（書籍章・スライド等の一覧：一杉研究室）**  
  https://solid-state-chemistry.jp/english/output_books.html  
  - 「材料科学のためのベイズ最適化の基礎知識」等の関連章がまとまっている。

- **（例）組成傾斜薄膜 × 自律探索（国内事例PDF）**  
  https://www.jst.go.jp/pr/announce/20251120-2/pdf/20251120-2.pdf  
  - 「材料探索（合成→評価→次条件提案）」にベイズ最適化がどう入るかの全体像が掴みやすい。
...
D = {(x1,y1),...,(xn,yn)}
repeat:
  surrogate = fit_probabilistic_model(D)   # 例：Gaussian Process
  x_next = argmax acquisition(x; surrogate) # 例：EI/UCB
  y_next = expensive_measurement_or_DFT(x_next)
  D = D ∪ {(x_next, y_next)}
until budget exhausted / convergence
```
