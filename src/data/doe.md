...
実験計画法（DOE）とは、限られた実験回数で因子（組成・プロセス・熱処理条件など）が特性に与える影響を、統計モデルとして切り分けて推定するための設計手法である。材料研究では、探索（スクリーニング）から最適化（応答曲面）までを、再現性と効率を両立して進める基盤となる。
