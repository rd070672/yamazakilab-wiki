# グランドカノニカルモンテカルロ（Grand Canonical Monte Carlo; GCMC）

GCMCは、温度Tと体積Vを固定しつつ、化学ポテンシャルμによって粒子数N（または組成）を揺らがせるモンテカルロ法である。材料科学では、吸着・欠陥・表面被覆・合金組成など「開放系」の平衡を、実験条件（分圧・活量・電位など）に対応づけて予測するための基盤手法である。
